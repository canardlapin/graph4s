...
sealed trait GraphLinalgError[+V]:
  def message: String

object GraphLinalgError:
  final case class InvalidWeight[V](
      edgeIndex: Int,
      from: V,
      to: V,
      value: Double,
      requirement: WeightRequirement
  ) extends GraphLinalgError[V]:
    def message: String =
      s"edge $edgeIndex ('$from','$to') has weight $value; expected ${requirement.label}"

  final case class ZeroStrengthVertex[V](vertex: V) extends GraphLinalgError[V]:
    def message: String =
      s"vertex '$vertex' has zero weighted strength"

  final case class EigenFailure(error: LinAlgError) extends GraphLinalgError[Nothing]:
    def message: String =
      error.getMessage

  final case class InvalidSpectrumRank(requested: Int, limit: Int)
  ...
  final case class InvalidEmbeddingDimensions(requested: Int, available: Int, dropped: Int)
      extends GraphLinalgError[Nothing]:
    def message: String =
      s"spectral embedding requested $requested dimension(s) after dropping $dropped eigenvector(s), but vertex order is $available"
